...
# Function to make sure codes are 5-digits
fix_codes <- function(x, req_length = 5){
  
  for (i in 1:length(x)){
    
    n <- str_length(x[i])
    missing <- req_length - n  
    
    if(missing > 0){
      x[i] <- paste0(rep("0", missing), x[i])
    }
...
mex_ports <- mex_ports_raw |> 
  ...
  # Ensure port id and municipality ID are all 5-digits long
  mutate(municipality_code = fix_codes(x = municipality_code,
                                       req_length = 5),
         port_id = fix_codes(x = port_id,
                             req_length = 5)) |> 
