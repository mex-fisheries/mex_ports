...
There are 151 ports in the final data. Lake ports, ports without a matching port in both files, and repeats are filtered out in 03_combine_sources.R.
155 individual ports are identified in mexican_ports.csv, 4 of which are on lakes and therefore are not coastline ports. 
51 of the total ports in mexican_ports.csv can be matched to ports in `mex_large_scale_landing_ports.csv` by landing site id, which is reflected in the mex_ports_dictionary.csv file. The ports that do not have a correlated landing site id will show NA in that colum. 
